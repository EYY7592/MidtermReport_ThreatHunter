...
```
IntelFusion（NVD + OTX + KEV + EPSS 並行查詢）
    ↓ fusion_results（已評分的 CVE 清單）
Scout（以套件名稱為單位，查 NVD + OTX）
    ↓ vulnerabilities[]
...
2. **每個套件必須呼叫 search_nvd 一次**（Step 2 強制要求）

3. **OTX 只對 CVSS ≥ 7.0 的 CVE 呼叫**（Step 3 conditional）
...
| 工具 | 對應檔案 | 誰在用 | 呼叫條件 |
|------|----------|--------|----------|
| `search_nvd` | `tools/nvd_tool.py` | IntelFusion + Scout | 每個套件必呼叫 |
| `search_otx` | `tools/otx_tool.py` | IntelFusion + Scout | CVSS ≥ 7.0 |
...
| `search_osv` | `tools/osv_tool.py` | IntelFusion（選用）| ecosystem-aware 查詢 |
...
| 檔案 | 修改目的 | 注意事項 |
|------|----------|----------|
| `skills/threat_intel.md` | 調整 Scout 查詢邏輯、OTX 門檻、Output Schema | 保留 Quality Redlines |
